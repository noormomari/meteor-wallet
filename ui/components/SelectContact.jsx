...
export const SelectContact = ({ selectedContact, setSelectedContact, title, contacts }) => {
          ...
          <div className="relative mt-2">
            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
              <span className="flex items-center">
               {(selectedContact?.imageUrl && <img src={selectedContact.imageUrl} alt="" className="h-5 w-5 flex-shrink-0 rounded-full" />)}
                <span className="ml-3 block truncate">{selectedContact?.name || 'Select a contact'}</span>
              </span>
                ...
                {contacts.map((contact) => (
                  ...
                  >
                    {({ selectedContact, active }) => (
                      <>
                        <div className="flex items-center">
                          {contact?.imageUrl && (<img src={contact.imageUrl} alt="" className="h-5 w-5 flex-shrink-0 rounded-full" />)}
                          <span
                            className={classNames(contact ? 'font-semibold' : 'font-normal', 'ml-3 block truncate')}
                          >
                            {contact.name}
                          </span>
                        </div>
